...
# ID import
qid_mappings <- read_csv(data_file)[2, ] # In R, if you don't skip, Row 3 is often index 2
...
keystroke_question_data <- data %>%
  select(ResponseId, keystroke_log) %>%
  ...
  ungroup() %>%
  select(ResponseId, qid, export_id, everything())
